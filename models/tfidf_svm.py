...
def main():
    ...
    for seed in range(1, 6):
        if config.task_type == 'multi_label':
            classifier = OneVsRestClassifier(LinearSVC(random_state=seed, max_iter=50000, dual=True))
            parameters = {
        ...
        if isinstance(x_train, list) and isinstance(x_val, list):
            x_train_val = x_train + x_val
        else:
            x_train_val = pd.concat([x_train, x_val])
        
        if config.task_type == 'multi_label':
            mlb = MultiLabelBinarizer(classes=list(LABEL_TO_ID.values()))
