...
def main():
    ...
    # BASE_DIR = f'/home/panzaresce/Desktop/unibo/year2/tesi/dataset_refactoring/logs/{config.dataset}'
    ...
    for model in MODELS:
        ...
        for seed in range(1, 6):
            ...
            try:
                ...
                print(f"{model}/{seed}/{'all_results.json'} NOT FOUND")
                continue
    ...
    for algo, stats in score_dicts.items():
        ...
        if config.filter_outliers:
            ...
            # Pick the model with the best scores for the eval dataset, so the best model on the test set may be different from the one showed
